...
export default function KanbanColumn({
  ...
  onDrop,
  onDragStart,
  shouldAnimate = false, // animate on initial mount if true
}) {
  ...
  // Keep previous values in refs so we can animate from previous -> current
  // If shouldAnimate is true on initial mount, start from 0
  const prevCountRef = useRef(shouldAnimate ? 0 : cards.length);
  const prevTotalRef = useRef(shouldAnimate ? 0 : totalValue);

  // Determine start and whether value actually changed
  const countStart = prevCountRef.current;
  const totalStart = prevTotalRef.current;
  const countChanged = countStart !== cards.length;
  const totalChanged = totalStart !== totalValue;

  // After render, update refs to current values (so next change is compared properly)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleAutoScroll = (e) => {
      ...
      const edgeThreshold = 100; // reasonable threshold for vertical autoscroll
