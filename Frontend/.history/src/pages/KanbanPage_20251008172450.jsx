...
export default function KanbanPage() {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      // Only start board drag if not clicking on a card or column content
      const target = e.target;
      const isCardElement = target.closest('[data-card-id]');
      const isColumnHeader = target.closest('[data-column-header]');
      
      if (isCardElement || isColumnHeader) return;
    ...
    const handleMouseMove = (e) => {
      ...
      board.scrollLeft = dragStart.scrollLeft - walk;
    };
    ...
    const handleTouchStart = (e) => {
      ...
      setIsDraggingBoard(true);
      setDragStart({
        x: e.touches[0].pageX - board.offsetLeft,
        scrollLeft: board.scrollLeft
      });
    };
