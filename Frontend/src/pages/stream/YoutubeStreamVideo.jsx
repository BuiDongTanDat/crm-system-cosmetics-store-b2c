...
export default function YoutubeStreamVideo() {
  ...
  const compactFileLabel = (f) => {
    ...
    const name =
      f.name.length > max
        ? `${f.name.slice(0, max - 3)}...${f.name.slice(-8)}`
        : f.name;
  ...
  const handleUploadOnly = async () => {
    ...
    try {
      ...
      if (!status?.authenticated) {
        ...
        try {
          ...
          console.error(
            "Failed to save campaign to sessionStorage before redirect",
            e
          );
        }
    ...
    try {
      ...
      if (!streamId) {
        ...
        streamId =
          createRes?.streamId || createRes?.id || createRes?.data?.streamId;
        if (!streamId) throw new Error("Không nhận được streamId từ server.");
        setActiveStreamId(streamId);
      } else {
        console.debug(
          "[YouTubeStream] Reusing existing streamId for upload:",
          streamId
        );
      }

      const uploadResp = await uploadStreamFile(streamId, selectedFile, {
        title: titleForStream,
        description,
      });
      ...
      toast.success(
        "Upload thành công. Bấm 'BẮT ĐẦU' để phát video lên YouTube."
      );
    } catch (err) {
  ...
  const handleStartStream = async () => {
    ...
    try {
      ...
      if (!watchUrlState) {
        try {
          const prep = await prepareYoutube(activeStreamId, {
            title: titleForStream,
            description,
          });
          ...
          // Nếu error là 'Redirecting to YouTube authentication...' thì không hiện toast error
          if (err.message === "Redirecting to YouTube authentication... ") {
            console.log("Redirecting to auth, skipping error toast");
            return; // Thoát sớm, không set starting = false
          }
          throw err; // Ném lại error khác
        }
      }

      const startResp = await startStreamFile(activeStreamId, {
        title: titleForStream,
        description,
      });
      ...
      console.error(err);
      toast.error(err.message || "Không thể bắt đầu stream");
    } finally {
      setStarting(false);
    }
  };
  ...
  const handleStopStream = async () => {
    if (!activeStreamId) {
      toast.error("Không có stream để dừng.");
      return;
    }
    try {
          ...
          <div className="flex items-center gap-3">
            ...
            {needsBackConfirm ? (
              <ConfirmDialog
                ...
                description={
                  <>
                    Hiện có file đã upload hoặc buổi phát đang chạy. Thoát sẽ
                    dừng/huỷ và xoá dữ liệu liên quan.
                  </>
                }
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 px-2 py-1">
                  ...
                  <Button
                    variant="outline"
                    onClick={() =>
                      document.getElementById("videoFileInput2")?.click()
                    }
                    disabled={isStreaming || uploading || starting}
                  >
                  ...
                  <Button
                    ...
                    disabled={
                      uploading || isStreaming || !selectedFile || starting
                    }
                  >
                    {uploading ? (
                  ...
                  {isStreaming ? (
                    <ConfirmDialog
                      title="Xác nhận dừng"
                      description={
                        <>Bạn có chắc chắn muốn dừng buổi phát trực tiếp?</>
                      }
                      confirmText="Dừng"
                      cancelText="Hủy"
                      onConfirm={handleStopStream}
                    >
                      <Button variant="actionDelete" disabled={starting}>
              ...
              <div className="w-full h-[220px] sm:h-[320px] bg-white border-2 border-dashed border-gray-200 flex items-center justify-center relative overflow-hidden rounded">
                ...
                {watchUrlState ? (
                  ...
                  <div className="text-sm text-gray-500">
                    Upload file và BẮT ĐẦU stream để xem review trực tiếp trên
                    youtube
                    <br />
                    1) Chọn file video từ máy tính.
                    <br />
                    2) Bấm "Upload" để tải video lên hệ thống.
                    <br />
                    3) Bấm "Bắt đầu" để phát trực tiếp video lên YouTube và xem
                    trước tại đây.
                  </div>
                )}
              </div>
              ...
              <Button
                onClick={() => {
                  if (watchUrlState) window.open(watchUrlState, "_blank");
                    ...
                    toast.error(
                      "Chưa có link xem live. Vui lòng upload và bấm Bắt đầu."
                    );
                }}
              ...
              <Button
                onClick={() => {
                  ...
                  if (vid) {
                    ...
                    window.open(
                      `https://studio.youtube.com/video/${vid}/live`,
                      "_blank"
                    );
                  } else {
            ...
            <div className="bg-white rounded-lg shadow p-4 flex-1 flex flex-col min-h-[200px]">
              <div className="grid gap-2 flex-1">
                {watchUrlState ? (
                    ...
                    if (!vid)
                      return (
                        <div className="text-xs text-gray-500">
                          Không thể xác định video id để hiển thị chat.
                        </div>
                      );
                  ...
                  <div className="text-xs text-gray-500">
                    Chưa có link YouTube để đồng bộ chat.
                  </div>
                )}
              </div>
            </div>
