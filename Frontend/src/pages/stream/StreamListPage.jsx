...
export default function StreamListPage() {
  ...
  const openViewModal = (campaign) =>
    setModal({ open: true, mode: "view", campaign });
  const openEditModal = (campaign) =>
    setModal({ open: true, mode: "edit", campaign });
  const openAddModal = () =>
    setModal({ open: true, mode: "edit", campaign: null });
  ...
  const openSelectStreamDialog = (campaign) =>
    setSelectDialog({ open: true, campaign });
  const closeSelectStreamDialog = () =>
    setSelectDialog({ open: false, campaign: null });
  ...
  const [campaigns, setCampaigns] = useState(() =>
    (Array.isArray(mockCampaigns) ? mockCampaigns : []).filter(
      (c) => c.type === "LiveStream"
    )
  );
  ...
  const handleSave = (campaignData) => {
    ...
    if (isCreating) {
      const maxId = campaigns.length
        ? Math.max(...campaigns.map((c) => c.id || 0))
        : 0;
      const newCampaign = {
        ...campaignData,
        id: maxId + 1,
      ...
      setCampaigns((prev) =>
        prev.map((c) =>
          c.id === campaignData.id ? { ...c, ...campaignData } : c
        )
      );
  ...
  return (
    <div className="flex flex-col">
      ...
      <div
        className="my-3 z-20 flex flex-col gap-3 px-6 py-3 bg-brand/10 backdrop-blur-lg rounded-md"
        style={{ backdropFilter: "blur" }}
      >
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          {/* Cụm trái: Tiêu đề + nút đổi chế độ */}
          <div className="flex items-center gap-2 justify-between w-full lg:justify-start">
            <h1 className="text-xl font-bold text-gray-900">
              Danh sách Stream ({filtered.length})
            </h1>
                        ...
                        >
                          ...
                          <PermissionGuard module="role" action="create">
                            <Button
                              variant="actionUpdate"
                              size="icon"
                              onClick={() => openSelectStreamDialog(c)}
                            >
                              <TvMinimalPlay className="w-4 h-4" />
                            </Button>
                          </PermissionGuard>
      ...
      <AppDialog
        ...
        title={{
          view: `Chi tiết chiến dịch - ${
            modal.campaign?.name || modal.campaign?.title || ""
          }`,
          edit: modal.campaign
