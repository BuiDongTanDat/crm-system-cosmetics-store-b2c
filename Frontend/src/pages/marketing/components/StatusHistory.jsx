...
export default function StatusHistory({ history }) {
    ...
    const getIcon = (action) => {
        switch (action) {
            case 'submit': return <Send className="w-4 h-4 text-blue-500" />;
    ...
    const getLabel = (action, status) => {
        switch (action) {
            case 'submit': return "Đã gửi duyệt";
    ...
    return (
        <div className="border rounded-lg p-4 bg-white">
            <h3 className="font-semibold mb-4 text-sm uppercase text-gray-500">Lịch sử hoạt động</h3>
            <div className="relative pl-4 space-y-6 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gray-100">
                {sorted.map((item, i) => (
                    <div key={i} className="relative pl-6">
                        <div className="absolute left-0 top-0.5 bg-white p-1 rounded-full border border-gray-100 shadow-sm z-10">
                            {getIcon(item.action)}
                        </div>
                        <div>
