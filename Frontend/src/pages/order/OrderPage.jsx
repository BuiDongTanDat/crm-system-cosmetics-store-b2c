...
export default function OrderPage() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesCustomer = selectedCustomer
      ? String(order.customer_id) === String(selectedCustomer)
      : true;
  ...
  useEffect(
    () => setCurrentPage(1),
    [searchTerm, selectedStatus, selectedCustomer, dateRange]
  );
  ...
  const handleCreate = () =>
    setModal({ open: true, mode: "create", order: null });
  ...
  const isOnlyStatusUpdate = (originalOrder, payload) => {
    ...
    if (
      keys.some(
        (k) =>
          forbiddenKeys.includes(k) && payload[k] !== (originalOrder[k] ?? null)
      )
    ) {
      return false;
    }
    ...
    const statusChanged =
      String(payload.status) !== String(originalOrder.status);
  ...
  const handleImportSuccess = (importedData) => {
    try {
      const processedOrders = importedData.map((item, index) => ({
        ...
        customerName:
          item["Tên khách hàng"] || item.customerName || "Khách hàng",
        orderDate:
          item["Ngày đặt hàng"] ||
          item.orderDate ||
              ...
              <DropdownOptions
                options={[
                  ...
                  ...ORDER_STATUSES_LIST.map((s) => ({
                    value: s,
                    label: STATUS_LABELS[s] || String(s).toUpperCase(),
                  })),
                ]}
                value={selectedStatus || ""}
                onChange={(val) => setSelectedStatus(val || null)}
                width="w-full flex-1 lg:w-80 "
                placeholder="Trạng thái"
              />

              <DropdownWithSearch
                items={[
                  { customer_id: "", full_name: "Tất cả khách hàng" },
                  ...customers,
                ]}
                itemKey={(c) => c.customer_id || c.id}
                renderItem={(c) =>
                  c.full_name || c.fullName || c.name || c.customer_id
                }
                filterFn={(c, s) =>
                  (c.full_name || c.fullName || c.name || c.customer_id || "")
                    .toString()
                ...
                onSelect={(c) => {
                  setSelectedCustomer(c?.customer_id || c?.id);
                  setCurrentPage(1);
                }}
                placeholder={
                  selectedCustomer
