...
const ProductCard = ({ p, isLiked = false, onToggleLike, onOrder }) => {
  // Map API fields
  ...
  const outOfStock =
    inventory <= 0 || ["OUT_OF_STOCK", "DISCONTINUED"].includes(statusRaw);

  return (
    <div
      className={`h-full rounded-sm border bg-white shadow-sm overflow-hidden flex flex-col hover:scale-102 hover:shadow-lg transition-transform duration-200 animate-fadeIn`}
    >
      ...
      <div className="p-4 flex-1 flex flex-col">
        <div className="flex justify-between items-center mb-2">
          ...
          <div>
            {p.inventory_qty !== undefined && (
              <span
                className={`rounded-full px-2.5 py-1 text-[12px] font-medium backdrop-blur border ${
                  p.inventory_qty > 0 &&
        ...
        {shortDesc && (
          <p className=" text-[12px] text-gray-500 line-clamp-2">{shortDesc}</p>
        )}
          ...
          <div className="mt-2 flex gap-2">
            <Star
              className={`
                ...
                duration-150
                h-auto
                ${isLiked ? "text-yellow-300 fill-yellow-300" : ""}`}
              onClick={() => onToggleLike?.(p)}
            />
            <Button
              variant="actionCreate"
              className="h-9 text-sm w-full flex-1"
              onClick={() => onOrder?.(p)}
              disabled={outOfStock}
            >
              Thêm vào giỏ hàng
