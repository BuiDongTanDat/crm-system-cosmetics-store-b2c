      ...
      <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-red-50 p-6 rounded-xl border border-red-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-red-900 mb-1">Tỷ lệ Churn</h3>
            <p className="text-2xl font-bold text-red-700">
              {summary ? (Number(summary.churn_rate_proxy) * 100).toFixed(1) + '%' : '12.5%'}
            </p>
            <span className="text-sm text-red-600">tổng khách nguy cơ</span>
          </div>
          <div className="bg-yellow-50 p-6 rounded-xl border border-yellow-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-yellow-900 mb-1">Khách hàng nguy cơ cao</h3>
            <p className="text-2xl font-bold text-yellow-700">
              {summary ? summary.high_risk_customers : '...'}
            </p>
          ...
          <div className="bg-orange-50 p-6 rounded-xl border border-orange-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-orange-900 mb-1">Doanh thu ảnh hưởng</h3>
            <p className="text-2xl font-bold text-orange-700">
              {summary ? (Number(summary.revenue_at_risk_12m) / 1000000).toFixed(1) + 'M' : '...'} VNĐ
            </p>
            <span className="text-sm text-orange-600">doanh thu CLV có thể mất</span>
          </div>
          <div className="bg-green-50 p-6 rounded-xl border border-green-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-green-900 mb-1">Khách hàng giữ lại</h3>
            <p className="text-2xl font-bold text-green-700">
              {summary ? (Number(summary.retention_proxy) * 100).toFixed(1) + '%' : '...'}
            </p>
            <span className="text-sm text-green-600">khách hàng ổn định</span>
          </div>
        </div>
      </div>
