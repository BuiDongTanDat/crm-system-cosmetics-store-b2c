...
export default function CustomerListPage() {
  ...
  const mapApiToUi = (item) => {
    ...
    const mapGender = (g) => {
      ...
      if (low === "female" || low === "f" || low === "nữ" || low === "nu")
        return "female";
      return "other";
    };
  ...
  const handleBackFromHistory = () =>
    setModal((prev) => ({ ...prev, showHistory: false }));
  ...
  const handleSave = async (customerData) => {
    ...
    try {
      ...
      if (isCreating) {
        ...
        if (res?.ok) {
          const updated = mapApiToUi(
            res.data ?? { ...payload, customer_id: customerData.id }
          );
          setCustomers((prev) =>
            prev.map((c) => (c.id === customerData.id ? updated : c))
          );
  ...
  const getTypeBadge = (type) => {
    const baseClass =
      "p-1 rounded-sm text-[12px] font-semibold w-[100px] text-center inline-block"; // Thêm shadow cho hiệu ứng nổi bật hơn

    // Định nghĩa các lớp gradient mới
    const vipGradientClass =
      "bg-gradient-to-r from-blue-400 to-indigo-500 text-white font-bold";
    const premiumGradientClass =
      "bg-gradient-to-r from-pink-400 to-yellow-500 text-white font-bold";
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1000px]">
                <thead className="bg-gray-50">
                  <tr>
                    {["Khách hàng", "Giới tính", "Email", "SĐT", "Nguồn KH", "Loại KH", ""].map(
                      (header) => (
                        <th
                          key={header}
