  ...
  return (
    <div className="p-0">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
        <h2 className="text-xl font-semibold mb-2">Customer Lifetime Value Analysis</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-emerald-50 p-6 rounded-xl border border-emerald-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-emerald-900 mb-1">CLV trung bình</h3>
            <p className="text-2xl font-bold text-emerald-700">
              {summary ? (Number(summary.avg_clv_12m) / 1000000).toFixed(1) + 'M' : '...'} VNĐ
            </p>
          </div>
          <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-blue-900 mb-1">CLV cao nhất</h3>
            <p className="text-2xl font-bold text-blue-700">
              {summary ? (Number(summary.max_clv_12m) / 1000000).toFixed(1) + 'M' : '...'} VNĐ
            </p>
          </div>
          <div className="bg-orange-50 p-6 rounded-xl border border-orange-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-orange-900 mb-1">Thời gian sống TB</h3>
            <p className="text-2xl font-bold text-orange-700">
              {summary?.avg_lifetime_months ? Math.round(summary.avg_lifetime_months) : '24'} tháng
            </p>
          </div>
          <div className="bg-pink-50 p-6 rounded-xl border border-pink-100 shadow-sm transition-all hover:shadow-md">
            <h3 className="font-semibold text-pink-900 mb-1">ROI trung bình</h3>
            <p className="text-2xl font-bold text-pink-700">
              {summary?.avg_roi ? Number(summary.avg_roi).toFixed(1) + 'x' : '4.8x'}
            </p>
          </div>
        </div>
      </div>
