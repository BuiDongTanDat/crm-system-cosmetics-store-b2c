...
export default function OrderDialog({ modal, closeModal, handleSave, handleDelete }) {
  return (
    <Dialog open={modal.open} onOpenChange={closeModal}>
      <DialogContent
        className="sm:max-w-6xl w-full max-h-[90vh] p-0 overflow-hidden
        ...
        <DialogHeader className="sticky top-0 z-10 bg-white border-b p-6 pb-4">
          <DialogTitle>
            {modal.mode === "view"
              ? "Chi tiết đơn hàng"
              : modal.mode === "edit"
              ? (modal.order ? "Sửa đơn hàng" : "Thêm đơn hàng mới")
              : "Thông tin đơn hàng"}
          </DialogTitle>
