...
class AIController { 
  static async healthCheck(req, res) {
    try {
      ...
      console.error('[AI] healthCheck failed:', err?.message || err);
      return res
        .status(500)
        .json(fail(asAppError(err, { status: 500, code: 'AI_HEALTH_FAILED' })));
    }
  }
  static async generate_email_content(req, res) {
    try {
      ...
      if (!input || typeof input !== 'object') {
        return res.status(400).json(
          fail({
            code: 'INVALID_INPUT',
            message:
              'Thiếu trường "input" trong body (vd: { input: {...}, options: {...} })',
          })
        );
      }
      ...
      console.error('[AI] generate_email_content failed:', err?.message || err);
  ...
  static async suggest_marketing_campaign(req, res) {
    try {
      ...
      console.error('[AI] suggest_marketing_campaign failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
  ...
  static async predict_churn(req, res) {
    try {
      ...
      console.error('[AI] predict_churn failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
  ...
  static async batch_predict_churn(req, res) {
    try {
      ...
      console.error('[AI] batch_predict_churn failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
  ...
  static async segment_customers(req, res) {
    try {
      ...
      console.error('[AI] segment_customers failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
  ...
  static async recommend_products(req, res) {
    try {
      ...
      console.error('[AI] recommend_products failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
  ...
  static async predict_churn_by_customer_id(req, res) {
    try {
      ...
      const features = await customerService.getCustomerChurnFeatures(customer_id);
      if (!features) {
      ...
      console.error('[AI] predict_churn_by_customer_id failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
  ...
  // -----------------------
  // Snapshot: compute + score + persist
  // POST /v1/ai/customers/:id/snapshot/score
  // -----------------------
  static async scoreCustomerSnapshot(req, res) {
    try {
      ...
      if (!customer_id) {
        return res.status(400).json(
          fail({
            code: 'INVALID_INPUT',
            message: 'Thiếu customer_id param (:id)',
          })
        );
      }

      const row = await SnapshotService.scoreWithAI(customer_id, snapshot_date || null);
      return res.status(200).json(ok(row));
    } catch (err) {
      console.error('[AI] scoreCustomerSnapshot failed:', err?.message || err);
      return res.status(500).json(
        fail(
          asAppError(err, {
            status: 500,
            code: 'AI_SNAPSHOT_SCORE_FAILED',
            message: 'Không thể score + lưu snapshot cho customer.',
          })
        )
      );
    }
