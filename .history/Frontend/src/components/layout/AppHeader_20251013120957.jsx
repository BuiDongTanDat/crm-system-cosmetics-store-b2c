...
export default function AppHeader() {
  ...
  return (
    <header className={`sticky top-5 z-30 flex items-center justify-between h-14 px-4 gap-4 transition-all duration-300 ${isScrolled
      ...
      {/* Notifications popup (backdrop + panel) */}
      {showNotifications && (
        <>
          <div
          ...
          />
          <div
            className="fixed top-20 right-4 z-50 w-96 max-h-[60vh] overflow-auto bg-white rounded-lg shadow-lg border"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b">
              <h3 className="font-semibold">Thông báo</h3>
              <button className="text-sm text-gray-500" onClick={() => setNotifList([])}>Xóa tất cả</button>
            </div>
                  ...
                  <div key={n.id} className={`p-3 flex items-start gap-3 ${n.read ? 'bg-white' : 'bg-blue-50'}`}>
                    <div className="flex-1">
                      ...
                      <div className="mt-2 flex gap-2">
                        {!n.read && (
                          <button onClick={() => markAsRead(n.id)} className="text-xs text-blue-600">Đánh dấu đã đọc</button>
                        )}
                        <button onClick={() => deleteNotification(n.id)} className="text-xs text-red-600">Xóa</button>
                      </div>
                    </div>
                  </div>
                ))
