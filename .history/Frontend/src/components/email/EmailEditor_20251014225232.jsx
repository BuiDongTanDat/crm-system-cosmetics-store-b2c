...
export default function EmailEditor({ content, onChange }) {
  ...
  // Thêm hàm để thêm text vào visual editor
  const insertText = () => {
    const newText = '<p style="color: #333; font-size: 16px; line-height: 1.6; margin: 10px 0;">Nhập nội dung của bạn tại đây...</p>';
    handleContentChange('html', localContent.html + newText);
  };
        ...
        <div className="lg:col-span-3">
          {editorMode === 'visual' ? (
            <div className="space-y-4">
              ...
              {/* Visual Preview - có thể chỉnh sửa */}
              <div className="border border-gray-300 rounded-lg p-4 min-h-[400px] bg-white">
                <div
                  contentEditable={true}
                  suppressContentEditableWarning={true}
                  onBlur={(e) => handleContentChange('html', e.currentTarget.innerHTML)}
                  dangerouslySetInnerHTML={{
                    __html: localContent.html || '<p style="color: #ccc; text-align: center; padding: 40px;">Nhấp vào đây để bắt đầu nhập nội dung hoặc chọn template...</p>',
                  }}
                  className="prose max-w-none outline-none"
                  style={{ fontFamily: 'Arial, sans-serif', minHeight: '360px' }}
                />
              </div>
            </div>
          ) : (
