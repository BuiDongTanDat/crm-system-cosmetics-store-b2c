  ...
  const getVietnameseData = (product) => {
    ...
    Object.entries(product).forEach(([key, value]) => {
      if (LABELS[key]) {
        ...
        // Giữ nguyên field đã là tiếng Việt hoặc không có trong mapping
        vietnameseProduct[key] = value;
      }
    });

    // Xử lý các field có nhiều tên khác nhau
    if (!vietnameseProduct["Mô tả ngắn"] && (product.shortDescription || vietnameseProduct.shortDescription)) {
      vietnameseProduct["Mô tả ngắn"] = product.shortDescription || vietnameseProduct.shortDescription;
    }
    
    if (!vietnameseProduct["Mua/tháng"] && (product.monthlySales || vietnameseProduct.monthlySales)) {
      vietnameseProduct["Mua/tháng"] = product.monthlySales || vietnameseProduct.monthlySales;
    }

    if (!vietnameseProduct["Tiến độ bán"] && (product.salesProgress || vietnameseProduct.salesProgress)) {
      vietnameseProduct["Tiến độ bán"] = product.salesProgress || vietnameseProduct.salesProgress;
    }

    if (!vietnameseProduct["Thông số"] && (product.specifications || vietnameseProduct.specifications)) {
      vietnameseProduct["Thông số"] = product.specifications || vietnameseProduct.specifications;
    }

    if (!vietnameseProduct["HDSD"] && (product.usage || vietnameseProduct.usage)) {
      vietnameseProduct["HDSD"] = product.usage || vietnameseProduct.usage;
    }

    if (!vietnameseProduct["Đánh giá"] && (product.reviews || vietnameseProduct.reviews)) {
      vietnameseProduct["Đánh giá"] = product.reviews || vietnameseProduct.reviews;
    }
    ...
    <div className="bg-white border-gray-200 overflow-hidden rounded-sm
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`ml-3 px-2 py-1 text-[10px] font-medium rounded-full whitespace-nowrap ${
              ...
              (product.status || displayProduct["Trạng thái"] || '').includes('Còn hàng')
        ...
        <p className="text-gray-600 text-sm mb-3 line-clamp-3">
          ...
          Mô tả: {product.shortDesc || displayProduct["Mô tả ngắn"] || product.description || displayProduct["Mô tả"] || product.shortDescription}
        </p>
          ...
          {Object.entries(displayProduct)
            .filter(([vietnameseKey]) =>
              ![
                ...
                'Ảnh',
                'Link sản phẩm',
                // Loại bỏ các field tiếng Anh dư thừa
                'shortDescription',
                'monthlySales', 
                'salesProgress',
                'specifications',
              ...
              val && val !== 'null' && val !== 'undefined' && val !== '' ? (
                <div key={vietnameseKey} className="flex justify-between">
                  ...
                  <span className="truncate text-right max-w-[120px]" title={val}>
                    {String(val).length > 20 ? `${String(val).substring(0, 20)}...` : val}
                  </span>
                </div>
              ) : null
            )}
