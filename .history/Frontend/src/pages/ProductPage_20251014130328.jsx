...
export default function ProductPage() {
	...
	// CSV parser: handles quoted fields and CRLF/LF
	...
	function handleFile(e) {
		...
		reader.onload = ev => {
			try {
				...
				const { headers, objs } = parseCSV(text);
				setHeadersOrder(headers);
				setProducts(objs);
				setModal({ open: true, mode: 'view', product: null });
			} catch (err) {
	...
	// Save from form (create/update)
	function handleSave(product, index) {
		setProducts(prev => {
			const copy = [...prev];
			if (typeof index === 'number') copy[index] = product;
			else copy.push(product);
			return copy;
		});
		setModal(prev => ({
			...prev,
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div className="bg-white p-4 rounded-lg shadow-md">
						...
						<ProductForm
							...
							onSave={(prod) => handleSave(prod, selected ? selected._idx : undefined)}
