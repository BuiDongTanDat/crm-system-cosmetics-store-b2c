...
export default function ProductPage() {
  ...
  const handleImportSuccess = (importedData) => {
    try {
      ...
      const rawHeaders = importedData.headers || [];
      const headers = rawHeaders.map(h => (typeof h === 'string' ? h.replace(/^\uFEFF/, '').trim() : h));
      ...
      const processedProducts = rows
        .map((item, index) => {
          ...
          for (const [key, viLabel] of Object.entries(productFieldMapping)) {
            ...
            if (Array.isArray(item)) {
              // item is an array -> use headers to find index, fallback to key match
              let idx = headers.indexOf(viLabel);
              if (idx === -1) idx = headers.indexOf(key);
              if (idx === -1 && !isNaN(Number(key))) idx = Number(key);
              if (idx !== -1 && idx < item.length) value = item[idx];
            } else if (item && typeof item === "object") {
              // item is an object -> use label or key
              value = item[viLabel] ?? item[key] ?? "";
            } else {
              // primitive (string) - no structure -> keep empty
              value = "";
            }
        ...
        // loại bỏ hàng rỗng (chỉ id hoặc tất cả field rỗng)
        .filter(p => Object.keys(p).some(k => k !== "id" && String(p[k]).trim() !== ""));

      if (processedProducts.length === 0) {
        alert("Không tìm thấy sản phẩm hợp lệ trong file nhập.");
        return;
      }
