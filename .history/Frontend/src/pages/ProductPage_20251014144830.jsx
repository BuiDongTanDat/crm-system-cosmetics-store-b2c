...
export default function ProductPage() {
  ...
  try {
    ...
    //Nếu dòng đầu tiên có key dài dính hết header → tách lại
    if (
      rows.length &&
      ...
      const validRows = rows.map((r) =>
        Object.values(r)[0].split(",").map((v) => v.trim())
      );

      // Biến mảng thành list object hợp lệ
    ...
    const processedProducts = rows.map((item, index) => {
      ...
      for (const [key, viLabel] of Object.entries(productFieldMapping)) {
        const raw = item[viLabel] ?? item[key] ?? "";

        // Chỉ làm sạch các trường tiền tệ / phần trăm để tránh làm hỏng URL / mô tả
