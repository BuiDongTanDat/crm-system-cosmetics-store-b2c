  ...
  const handleImportSuccess = (importedData) => {
    try {
      ...
      const processedProducts = rows.map((item, index) => {
        ...
        const discountValue = cleanValue(discountText);

        return {
          id: Math.max(...products.map(p => p.id), 0) + index + 1,
          name: item['Tên sản phẩm'] || item.name || 'Sản phẩm chưa đặt tên',
          brand: item['Thương hiệu'] || '',
          currentPrice: cleanValue(item['Giá hiện tại']),
          originalPrice: cleanValue(item['Giá gốc']),
          ...
          source: item['Nguồn'] || '',
          currentPriceExtra: cleanValue(item['Giá hiện tại_extra']),
