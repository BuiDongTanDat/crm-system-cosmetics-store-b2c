...
export default function ProductPage() {
  ...
  const handleImportSuccess = (importedData) => {
    try {
      ...
      // Tự động xác định kiểu dữ liệu (PapaParse có thể trả về { data: [...] } hoặc mảng object)
      const rows = Array.isArray(importedData?.data)
        ? importedData.data
        : Array.isArray(importedData)
          ? importedData
          : [];

      console.log("Parsed rows (sau khi chuẩn hóa):", rows);
      ...
      // Ánh xạ từ header tiếng Việt sang key tiếng Anh trong code
      const processedProducts = rows.map((item, index) => {
        ...
        for (const [key, viLabel] of Object.entries(productFieldMapping)) {
          obj[key] = item[viLabel] ?? item[key] ?? "";
        }
        return obj;
      });

      console.log("✅ Processed products (sau khi map):", processedProducts);
      console.groupEnd();
      ...
      alert("Có lỗi xảy ra khi đọc dữ liệu CSV, vui lòng kiểm tra lại file.");
    }
  };
