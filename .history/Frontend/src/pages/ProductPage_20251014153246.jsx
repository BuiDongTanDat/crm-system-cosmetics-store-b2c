  ...
  const handleImportSuccess = (importedData) => {
    try {
      ...
      const processedProducts = rows.map((item, index) => {
        ...
        // Xử lý discount
        const discountRaw = getFieldValue('Giảm giá', 'discount');
        const discountValue = cleanNumber(discountRaw);

        const newProduct = {
          id: Math.max(...products.map(p => p.id), 0) + index + 1,
          name: getFieldValue('Tên sản phẩm', 'name', `Sản phẩm ${index + 1}`),
          ...
          shortDesc: getFieldValue('Mô tả ngắn', 'shortDesc'),
          rating: parseFloat(getFieldValue('Đánh giá sao', 'rating')) || 0,
          reviewCount: parseInt(getFieldValue('Số lượt đánh giá', 'reviewCount')) || 0,
          soldPerMonth: getFieldValue('Mua/tháng', 'soldPerMonth'),
          progress: getFieldValue('Tiến độ bán', 'progress'),
          giftOffer: getFieldValue('Ưu đãi/Quà tặng', 'giftOffer'),
          source: getFieldValue('Nguồn', 'source'),
          currentPriceExtra: cleanNumber(getFieldValue('Giá hiện tại_extra', 'currentPriceExtra')),
          description: getFieldValue('Mô tả', 'description'),
          specs: getFieldValue('Thông số', 'specs'),
          howToUse: getFieldValue('HDSD', 'howToUse'),
          ingredients: getFieldValue('Thành phần', 'ingredients'),
          fullReview: getFieldValue('Đánh giá', 'fullReview'),
