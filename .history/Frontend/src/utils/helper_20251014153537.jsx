...
// --- Đọc file CSV và trả về mảng dữ liệu JSON ---
// Hỗ trợ UTF-8 có BOM, dấu phẩy / chấm phẩy / tab, và giữ nguyên mô tả nhiều dòng
export async function importFromCSV(file) {
  ...
  // ⚙️ Regex tách dòng chuẩn CSV (không cắt mô tả nhiều dòng trong dấu "")
  ...
  // Kiểm tra xem có thực sự có dữ liệu hay không
  const nonEmpty = data.filter(
    (row) => Object.values(row).some((val) => val !== "")
  );

  if (nonEmpty.length === 0) {
    console.warn("⚠️ File CSV đọc được nhưng toàn dòng trống hoặc sai delimiter:", delimiter);
  }
